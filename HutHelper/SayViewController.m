...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    if ((indexPath.row==1&&photo.count==0)||(indexPath.row==2&&photo.count!=0)) {
        ...
        NSString *Url_String=[NSString stringWithFormat:@"",user.data.studentKH,user.remember_code_app,[self getsayid:indexPath.section]];
...
-(NSString*)getPhoto:(int)i with:(int)j{
    ...
    NSString *Url=[NSString stringWithFormat:@"http://hut.wxz.name:8888/%@",photo[j]];
    NSLog(@"请求地址%@",Url);
    return Url;
}
...
-(UIImage*)getImg:(int)i{
    NSString *Url=[NSString stringWithFormat:@"http://hut.wxz.name:8888/%@",[_Say_content[i] objectForKey:@"head_pic_thumb"]];
    if ([Url isEqualToString:@"http://hut.wxz.name:8888/"]) {
