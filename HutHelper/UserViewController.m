...
- (void)viewDidLoad {
    ...
    if ([defaults objectForKey:@"head_img"]!=NULL) {
        ...
        image_url=[@"http://hut.wxz.name:8888/" stringByAppendingString:image_url];
        NSURL *url                   = [NSURL URLWithString: image_url];//接口地址
        NSData *data = [NSData dataWithContentsOfURL:url];
        if (data!=NULL&&![image_url isEqualToString:@"http://hut.wxz.name:8888/"]) {
...
- (void)postimage:(UIImage *)img
{
    ...
    NSString *url_String=[NSString stringWithFormat:@"",user.data.studentKH,user.remember_code_app];
